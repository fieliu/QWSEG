# DINOv3 + ViT-Adapter + Mask2Former: RGB-only baseline on MFNet.
# Following official ViT-Adapter config pattern from czczup/ViT-Adapter.
# Architecture: EncoderDecoder + DINOv3Adapter backbone + Mask2FormerHead.
# Uses the first 3 channels of the 6-channel RGB-T input.
_base_ = [
...
custom_imports = dict(
    imports=['mmseg.models.backbones.dinov3_adapter',
             'mmseg.engine',
...
# Official ViT-Adapter backbone config (ViT-B scale)
# Reference: mask2former_beit_adapter_base_512_40k_cocostuff10k_ss.py
model = dict(
    type='EncoderDecoder',
...
# Reference: official ViT-Adapter uses layer_decay_rate=0.95 for ViT-B (12 layers)
optimizer = dict(
    type='AdamW',
    lr=3e-5,
    betas=(0.9, 0.999),
