"""
RGB-T语义分割 - 验证实验: 全量微调 (无LoRA)

目的: 验证全量微调在480x480分辨率下的性能上限，与LoRA基线公平对比
方法: 使用原始EncoderDecoder架构，所有参数都可训练
预期: 
  - 如果mIoU接近0.59，说明V1架构没问题，LoRA是瓶颈
  - 如果mIoU远低于0.59，说明480x480分辨率或训练epoch不够

架构: 与原始全量微调模型完全一致 (EncoderDecoder)
  - 输入: 6通道(RGB+Thermal)
  - Backbone: MAE ViT-B (共享权重，batch维度拼接RGB和IR)
  - 融合: 通道拼接 (view reshape)
  - 解码器: UPerNet

训练: 80 epoch (与LoRA实验相同epoch数，公平对比)
"""
