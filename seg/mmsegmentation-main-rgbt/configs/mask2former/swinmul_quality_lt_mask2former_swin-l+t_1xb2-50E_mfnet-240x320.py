...
custom_imports = dict(
    imports=['mmseg.models.segmentors.swin_quality_lt_mask2former',
             'mmseg.engine',
...
# ---------------------------------------------------------------------------
# Common branch: Swin-L (embed_dims=192, depths=[2,2,18,2])
# ---------------------------------------------------------------------------
swin_common = dict(
    ...
    init_cfg=dict(
        type='Pretrained',
        checkpoint='./pretrain/swin_large_patch4_window7_224_22k.pth'))

# ---------------------------------------------------------------------------
# Private branches: Swin-T (embed_dims=96, depths=[2,2,6,2])
# ---------------------------------------------------------------------------
swin_private = dict(
    type='SwinTransformer',
    embed_dims=96,
...
# ---------------------------------------------------------------------------
# Mask2Former head: in_channels aligned with Swin-L output
# [192, 384, 768, 1536]
# ---------------------------------------------------------------------------
...
model = dict(
    type='QualityGatedSwinLTMask2Former',
...
# ---------------------------------------------------------------------------
common_depths = [2, 2, 18, 2]
priv_depths = [2, 2, 6, 2]

backbone_norm_multi = dict(lr_mult=0.1, decay_mult=0.0)
backbone_embed_multi = dict(lr_mult=0.1, decay_mult=0.0)
custom_keys = {
    # Common branch (Swin-L): smaller lr
    'backbone': dict(lr_mult=0.1, decay_mult=1.0),
    'backbone.patch_embed.norm': backbone_norm_multi,
    'backbone.norm': backbone_norm_multi,
    'absolute_pos_embed': backbone_embed_multi,
    'relative_position_bias_table': backbone_embed_multi,
    # Private branches (Swin-T): smaller lr
    'private_branch_rgb': dict(lr_mult=0.1, decay_mult=1.0),
    'private_branch_t': dict(lr_mult=0.1, decay_mult=1.0),
}
# Common branch norm keys
custom_keys.update({
    f'backbone.stages.{stage_id}.blocks.{block_id}.norm': backbone_norm_multi
    for stage_id, num_blocks in enumerate(common_depths)
    for block_id in range(num_blocks)
})
custom_keys.update({
    f'backbone.stages.{stage_id}.downsample.norm': backbone_norm_multi
    for stage_id in range(len(common_depths) - 1)
})
# Private RGB branch norm keys
custom_keys.update({
    f'private_branch_rgb.stages.{stage_id}.blocks.{block_id}.norm': backbone_norm_multi
    for stage_id, num_blocks in enumerate(priv_depths)
    for block_id in range(num_blocks)
})
custom_keys.update({
    f'private_branch_rgb.stages.{stage_id}.downsample.norm': backbone_norm_multi
    for stage_id in range(len(priv_depths) - 1)
})
# Private T branch norm keys
custom_keys.update({
...
# Projection layers: higher lr since they are new
custom_keys.update({
    'priv_proj_rgb': dict(lr_mult=5.0, decay_mult=1.0),
    'priv_proj_t': dict(lr_mult=5.0, decay_mult=1.0),
})
