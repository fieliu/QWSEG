...
# Self-contained config inheriting the proven FMB dataset base directly
# (NOT the MFNet config) so the FMB directory-scan loading is used as-is.
# This is the M0 (full model) reference for FMB ablations.
_base_ = [
...
swin_common = dict(
    ...
    embed_dims=96, depths=depths, num_heads=[3, 6, 12, 24],
    window_size=7, mlp_ratio=4, qkv_bias=True, qk_scale=None,
    drop_rate=0., attn_drop_rate=0., drop_path_rate=0.3,
    patch_norm=True, out_indices=(0, 1, 2, 3), with_cp=False, frozen_stages=-1,
...
_mask2former_head = dict(
    ...
    strides=[4, 8, 16, 32],
    feat_channels=256, out_channels=256,
    num_classes=num_classes, num_queries=100, num_transformer_feat_level=3,
    align_corners=False,
    pixel_decoder=dict(
        type='mmdet.MSDeformAttnPixelDecoder', num_outs=3,
        norm_cfg=dict(type='GN', num_groups=32), act_cfg=dict(type='ReLU'),
        encoder=dict(
            num_layers=6,
            layer_cfg=dict(
                self_attn_cfg=dict(
...
_aux_head = dict(
    type='SegformerHead',
    in_channels=[96, 192, 384, 768], in_index=[0, 1, 2, 3], channels=256,
    dropout_ratio=0.1, num_classes=num_classes,
...
model = dict(
    ...
    common_decode_head=_aux_head,
    rgb_private_decode_head=_aux_head,
    t_private_decode_head=_aux_head,
    retention_min=0.4, retention_max=0.98, retention_loss_weight=2.0,
    phase_mode='absolute', phase1_epochs=10, phase2_epochs=20, total_epochs=200,
    loss_align_weight=0.1, contrast_tau=0.07, contrast_num_samples=512,
    loss_invariant_weight=0.03, loss_distill_weight=0.3, distill_temperature=4.0,
    aux_loss_weight=0.3, missing_ratio=0.3, global_deg_ratio=0.3,
    local_deg_ratio=0.4, tau=0.3, alpha=10.0, tau_hard=0.2,
    fuse_epsilon=1e-3, fuse_beta=6.0,
...
for _pfx in ['backbone', 'private_branch_rgb', 'private_branch_t']:
    custom_keys.update({
        f'{_pfx}.stages.{s}.blocks.{b}.norm': backbone_norm_multi
        for s, nb in enumerate(depths) for b in range(nb)})
    custom_keys.update({
        f'{_pfx}.stages.{s}.downsample.norm': backbone_norm_multi
...
for pred_name in ['predictors_common_rgb', 'predictors_common_t',
    ...
    for s in range(4):
        for sub in ['ctx_proj', 'norm1', 'conv1', 'norm2', 'conv2', 'score_head']:
            predictor_keys[f'{pred_name}.{s}.{sub}'] = dict(lr_mult=5.0, decay_mult=1.0)
