...
_mask2former_head = dict(
    ...
    pixel_decoder=dict(
        ...
        encoder=dict(
            num_layers=6,
            layer_cfg=dict(
                self_attn_cfg=dict(embed_dims=256, num_heads=8, num_levels=3,
    ...
    transformer_decoder=dict(
        ...
        layer_cfg=dict(
            self_attn_cfg=dict(embed_dims=256, num_heads=8, attn_drop=0.0,
                               proj_drop=0.0, dropout_layer=None, batch_first=True),
            cross_attn_cfg=dict(embed_dims=256, num_heads=8, attn_drop=0.0,
                                proj_drop=0.0, dropout_layer=None, batch_first=True),
    ...
    loss_mask=dict(type='mmdet.CrossEntropyLoss', use_sigmoid=True,
                   reduction='mean', loss_weight=5.0),
    loss_dice=dict(type='mmdet.DiceLoss', use_sigmoid=True, activate=True,
...
for branch in ['rgb_branch', 'thr_branch']:
    custom_keys.update({
        f'backbone.{branch}.stages.{sid}.blocks.{bid}.norm': backbone_norm_multi
        for sid, nb in enumerate(depths) for bid in range(nb)})
    custom_keys.update({
        f'backbone.{branch}.stages.{sid}.downsample.norm': backbone_norm_multi
        for sid in range(len(depths) - 1)})

# QualityPredictor LR = 5.0× backbone LR
...
param_scheduler = [
    ...
    dict(type='PolyLR', eta_min=0.0, power=0.9, begin=1500, end=117600, by_epoch=False),
]
