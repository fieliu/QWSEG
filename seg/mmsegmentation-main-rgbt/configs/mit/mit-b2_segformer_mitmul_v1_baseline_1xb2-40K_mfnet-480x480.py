"""
RGB-T语义分割 - MiT V1 Baseline (MiT-B2 + 交叉注意力融合) - MFNet 480x480

配置文件说明：
- 通用编码器：MixVisionTransformer-B2，RGB和Thermal共享权重（batch拼接）
- 融合：跨模态交叉注意力融合
- 分割头：SegformerHead
- 损失：CrossEntropyLoss
- 训练数据：仅干净数据
...
custom_imports = dict(
    imports=['mmseg.models.segmentors.mitmul_v1_baseline',
