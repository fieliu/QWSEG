"""
RGB-T语义分割 - 验证实验: SAM冻结Backbone + 只训练解码器

目的: 验证SAM预训练编码器的原始特征提取能力
方法: 使用原始EncoderDecoder架构，冻结backbone全部参数，只训练neck+decode_head+auxiliary_head
预期: 
  - 如果mIoU接近LoRA微调版本，说明SAM编码器特征质量好，LoRA是瓶颈
  - 如果mIoU远低于LoRA微调，说明SAM编码器特征需要更多微调
  - 与MAE冻结版本对比，可以评估SAM vs MAE的原始特征质量

架构: 与原项目EncoderDecoder一致
  - 输入: 6通道(RGB+Thermal)，backbone内batch拼接共享参数
  - Backbone: SAM ViT-B (in_channels=3, 共享权重处理RGB和IR)
...
custom_imports = dict(
    imports=['mmseg.models.backbones.sam_vit',
             'mmseg.datasets.fmb',
