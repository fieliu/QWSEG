"""
RGB-T语义分割 - V1 SAM Baseline (SAM ViT-B + LoRA + FocalLoss + DiceLoss) - FMB 768x768

配置文件说明：
- 通用编码器：SAM ViT-B (ImageEncoderViT)，RGB和Thermal共享权重
- 微调方式：LoRA (rank=16, alpha=16.0) - qkv, proj, lin1, lin2
...
- 输入尺寸：768x768
- 推理：滑动窗口
- 与v1 MAE基线唯一区别：主干从MAE ViT-B替换为SAM ViT-B
"""
...
model = dict(
    ...
    lora_alpha=16.0,
    lora_dropout=0.1,
    lora_target_modules=['q_proj', 'v_proj', 'proj', 'lin1', 'lin2'],
