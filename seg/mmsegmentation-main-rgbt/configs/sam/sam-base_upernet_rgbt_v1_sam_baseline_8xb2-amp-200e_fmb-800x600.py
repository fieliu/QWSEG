"""
RGB-T语义分割 - V1 SAM Baseline (SAM ViT-B + LoRA + 交叉注意力融合) - FMB 800x600

配置文件说明：
- 通用编码器：SAM ViT-B (ImageEncoderViT)，RGB和Thermal共享权重
- 微调方式：LoRA (rank=4, alpha=4.0) - qkv, proj
- 融合：交叉注意力机制 (Cross-Attention Fusion)
  - RGB->T: RGB作为query, T作为key/value
  - T->RGB: T作为query, RGB作为key/value
  - 门控融合: 学习自适应权重融合两个增强特征
- 分割头：UPerNet解码器
...
_base_ = [
    '../_base_/datasets/fmb_800x600.py',
...
model = dict(
    ...
    test_cfg=dict(mode='slide', crop_size=crop_size, stride=(300, 400)))
