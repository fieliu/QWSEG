...
class RGBTPhotoMetricDistortion(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = results['img']
        rgb_deg = results.get('rgb_degradation', 'clean')
        thermal_deg = results.get('thermal_degradation', 'clean')
        rgb_missing = (rgb_deg == 'missing')
        thermal_missing = (thermal_deg == 'missing')

        img = self._distort_with_missing(img, rgb_missing, thermal_missing)

        results['img'] = img
        return results

    def _distort_with_missing(self, img, rgb_missing, thermal_missing):
        img = img.copy()
        if rgb_missing and thermal_missing:
            return img

        if rgb_missing:
            img[:, :, 3:] = self._distort_single(img[:, :, 3:])
            return img
    ...
    def _distort_single(self, img_single):
        tmp = np.concatenate([img_single, img_single], axis=2)
        tmp = self.brightness(tmp)
        mode = random.randint(2)
        if mode == 1:
            tmp = self.contrast(tmp)
