            ...
            if attn_mask is None:
                ...
                if attn_mask.ndim == 3:
                    ...
                    attn_mask = attn_mask.unsqueeze(1).repeat(
                        extra_attn_mask.shape[0] // nW, extra_attn_mask.shape[1], 1, 1)
                if attn_mask.shape != extra_attn_mask.shape:
                    attn_mask = attn_mask.expand_as(extra_attn_mask)
                attn_mask = attn_mask + extra_attn_mask
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape, attn_mask=None):
        ...
        def _inner_forward(x):
            identity = x
            x = self.norm1(x)
            x = self.attn(x, hw_shape, extra_attn_mask=attn_mask)
...
class SwinTransformer(BaseModule):
    ...
    def forward(self, x, stage_attn_masks=None):
        ...
        for i, stage in enumerate(self.stages):
            mask_i = None
            if stage_attn_masks is not None and i < len(stage_attn_masks):
                mask_i = stage_attn_masks[i]
