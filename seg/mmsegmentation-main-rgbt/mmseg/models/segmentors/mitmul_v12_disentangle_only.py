...
def _forward_mit_no_quality(backbone, img):
    outs = []
    ...
    for i, layer in enumerate(backbone.layers):
        ...
        for block in blocks:
            ...
            residual = x
            x = block.norm2(x)
            ffn_out = nlc_to_nchw(x, hw_shape)
...
class MiTMulV12DisentangleOnly(BaseSegmentor):
    ...
    def _extract_feat_single(self, input_rgb, input_t):
        ...
        zc_rgbt_list = _forward_mit_no_quality(self.backbone, input_rgbt)

        zc_rgb_list = [feat[:B] for feat in zc_rgbt_list]
        zc_t_list = [feat[B:] for feat in zc_rgbt_list]

        zp_rgb_list = _forward_mit_no_quality(
            self.private_branch_rgb, input_rgb)
        zp_t_list = _forward_mit_no_quality(
