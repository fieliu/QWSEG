...
class CrossAttentionEnhance(BaseModule):
    ...
    def forward(self, private_feat, common_feat, quality_weight_1d=None):
        B, C, H, W = private_feat.shape
        N = H * W

        priv_nlc = nchw_to_nlc(private_feat)
        comm_nlc = nchw_to_nlc(common_feat)
        ...
        if quality_weight_1d is not None:
            residual = residual * quality_weight_1d.unsqueeze(-1)

        return nlc_to_nchw(residual, (H, W))
...
def _get_quality_weight_1d(q_2d, threshold, H, W, B, orig_B=None,
                           q_other_2d=None, min_weight=0.1):
    if q_2d.shape[2:] != (H, W):
    ...
    if orig_B is not None and B == orig_B * 2:
        ...
        rgb_w[rgb_low & ~both_low] = min_weight
        t_w[t_low & ~both_low] = min_weight

        rgb_w[both_low & rgb_better] = torch.clamp(rgb_w[both_low & rgb_better], min=threshold)
        t_w[both_low & rgb_better] = min_weight
        rgb_w[both_low & ~rgb_better] = min_weight
        t_w[both_low & ~rgb_better] = torch.clamp(t_w[both_low & ~rgb_better], min=threshold)

        weight = torch.cat([rgb_w, t_w], dim=0)
        ...
        w = q_s.clone()
        w[cur_low & ~both_low] = min_weight
        w[both_low & cur_better] = torch.clamp(w[both_low & cur_better], min=threshold)
        w[both_low & ~cur_better] = min_weight

        weight = w
    else:
        weight = q_s.clone()
        weight[q_s < threshold] = min_weight

    weight_1d = weight.reshape(B, -1)
    return weight_1d


def _forward_mit_with_quality(backbone, img, q_maps, threshold, orig_B=None,
                             q_other_maps=None):
    outs = []
    ...
    for i, layer in enumerate(backbone.layers):
        ...
        if i < len(q_maps):
            q_i = q_maps[i]
            if q_i.shape[2:] != hw_shape:
                q_i = F.interpolate(q_i, size=hw_shape, mode='nearest')
            q_i_other = None
            if q_other_maps is not None and i < len(q_other_maps):
                q_i_other = q_other_maps[i]
                if q_i_other.shape[2:] != hw_shape:
                    q_i_other = F.interpolate(
                        q_i_other, size=hw_shape, mode='nearest')
            quality_weight_1d = _get_quality_weight_1d(
                q_i, threshold, hw_shape[0], hw_shape[1], x.shape[0],
        ...
        for j, block in enumerate(blocks):
            ...
            attn_out = attn_module.attn(query=x_q, key=x_kv, value=x_kv)[0]

            if attn_module.batch_first:
                attn_out = attn_out.transpose(0, 1)

            attn_out = attn_module.dropout_layer(
                attn_module.proj_drop(attn_out))
...
class MiTMulV12DQualityDisentangle(BaseSegmentor):
    ...
    def _extract_feat_single(self, input_rgb, input_t):
        ...
        for i in range(num_stages):
            ...
            rgb_weight = _get_quality_weight_1d(
                q_rgb_i, self.quality_threshold, H, W, B,
                q_other_2d=q_t_i)
            t_weight = _get_quality_weight_1d(
                q_t_i, self.quality_threshold, H, W, B,
                q_other_2d=q_rgb_i)

            rgb_enh = self.cross_attn_rgb[i](
                zp_rgb_list[i], zc_fused, quality_weight_1d=rgb_weight)
            t_enh = self.cross_attn_t[i](
                zp_t_list[i], zc_fused, quality_weight_1d=t_weight)
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        if torch.isnan(losses.get('final.loss_mask', torch.tensor(0.0))):
            for i, feat in enumerate(final_neck):
                has_nan = torch.isnan(feat).any().item()
                has_inf = torch.isinf(feat).any().item()
                print(f'[NaN-DEBUG] final_neck[{i}] shape={feat.shape} '
                      f'range=[{feat.min().item():.4f}, {feat.max().item():.4f}] '
