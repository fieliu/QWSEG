...
def compute_quality_guided_alignment(zc_rgb, zc_t, q_rgb, q_t,
                                      temperature=0.07):
...
class MiTMulV5QualityJoint(BaseSegmentor):
    ...
    def _apply_token_pruning(self, tokens, quality_scores, hw_shape):
        ...
        if strategy == 'soft_mask':
            ...
            keep_mask = (quality_scores >=
                         self.quality_prune_threshold).float()
            pruned_tokens = tokens * keep_mask.unsqueeze(-1)
            return pruned_tokens, keep_mask
    ...
    def extract_feat_train(self, inputs: torch.Tensor):
        ...
        for i in range(num_stages):
            ...
            pruned_zp_rgb, rgb_weights = self._apply_token_pruning(
                zp_rgb_tokens, q_rgb, hw_shapes[-1])
            pruned_zp_t, t_weights = self._apply_token_pruning(
                zp_t_tokens, q_t, hw_shapes[-1])
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        (zc_rgb_list, zc_t_list, zp_rgb_list, zp_t_list,
         ...
         q_rgb, q_t) = self.extract_feat_train(input_rgbt)
        ...
        if rgb_clean_mask.any() and rgb_degraded_mask.any():
            loss_c, loss_d = self.quality_anchor_fn(
                q_rgb[rgb_clean_mask], q_rgb[rgb_degraded_mask])
            loss_anchor = loss_anchor + loss_c + loss_d
        elif rgb_clean_mask.any():
            loss_c, _ = self.quality_anchor_fn(q_rgb[rgb_clean_mask])
            loss_anchor = loss_anchor + loss_c

        if thermal_clean_mask.any() and thermal_degraded_mask.any():
            loss_c, loss_d = self.quality_anchor_fn(
                q_t[thermal_clean_mask], q_t[thermal_degraded_mask])
            loss_anchor = loss_anchor + loss_c + loss_d
        elif thermal_clean_mask.any():
            loss_c, _ = self.quality_anchor_fn(q_t[thermal_clean_mask])
            loss_anchor = loss_anchor + loss_c

        losses['loss_anchor'] = loss_anchor * 0.5
