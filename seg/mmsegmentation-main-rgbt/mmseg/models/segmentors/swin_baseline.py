...
class ConcatMLPFusion(nn.Module):
    def __init__(self, in_channels_list, out_channels_list):
        ...
        for in_ch, out_ch in zip(in_channels_list, out_channels_list):
            num_groups = min(32, out_ch) if out_ch % 32 != 0 else 32
    ...
    def forward(self, rgb_feats, t_feats):
        fused = []
        for i in range(len(rgb_feats)):
            x = torch.cat([rgb_feats[i], t_feats[i]], dim=1)
            x = self.fusions[i](x)
            fused.append(x)
        return fused
...
class SwinBaseline(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.fusion = ConcatMLPFusion(in_channels_list, in_channels_list)
    ...
    def extract_feat(self, inputs: torch.Tensor):
        ...
        fused_feats = self.fusion(x_rgb_list, x_t_list)
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        loss_decode = self.decode_head.loss(
            fused_feats, data_samples, self.train_cfg)
