"""Quality-Gated Swin Hard Mask Variant.

Pure hard-constraint model:
  - QualityPredictor outputs 2-class logits per spatial position
  - Gumbel-Softmax converts to hard 0/1 mask (STE for gradient)
  - Common branch: complementary mask ensures at least one modality survives
  - Attention: hard bias for masked-out positions (approaching zero attention)
  - Feature output: multiply by hard mask to zero out low-quality features
  - Fusion: direct summation (no normalization, no quality weighting)
  - Final fusion: sum(common_fused, priv_rgb, priv_t) → 1x1 conv → norm
  - Cross-stage mask accumulation: max_pool previous mask, multiply with current
...
def _hard_mask_to_swin_bias(D_2d, window_size, shift_size, neg_bias=-100.0):
    B, _, H, W = D_2d.shape
    bias_map = torch.where(D_2d > 0.5, torch.zeros_like(D_2d),
                           torch.full_like(D_2d, neg_bias))
...
class QualityPredictorGumbel(nn.Module):
    def __init__(self, in_channels, init_keep_bias=3.0):
        super().__init__()
        ...
        nn.init.constant_(self.gate_head.bias[0], init_keep_bias)
        nn.init.constant_(self.gate_head.bias[1], -init_keep_bias)
        self._init_weights()
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None and m is not self.gate_head:
...
def _forward_swin_common_hard(swin_branch, input_rgbt, orig_B,
                               predictors_rgb, predictors_t,
                               training=True, gumbel_tau=1.0,
                               attn_neg_bias=-100.0):
    window_size = swin_branch.stages[0].blocks[0].attn.window_size
    outs, all_D_rgb, all_D_t, all_y_rgb, all_y_t = [], [], [], [], []
    ...
    for i, stage in enumerate(stages):
        ...
        if i < len(predictors_rgb) and predictors_rgb[i] is not None:
            ...
            logits_rgb = predictors_rgb[i](x_2d[:orig_B])
            logits_t = predictors_t[i](x_2d[orig_B:])
            D_rgb, y_rgb = gumbel_softmax_hard(logits_rgb, tau=gumbel_tau, training=training)
            D_t, y_t = gumbel_softmax_hard(logits_t, tau=gumbel_tau, training=training)

        if D_rgb is None:
            D_rgb = torch.ones(orig_B, 1, H, W, device=x.device, dtype=x.dtype)
            y_rgb = D_rgb.clone()
        if D_t is None:
            D_t = torch.ones(orig_B, 1, H, W, device=x.device, dtype=x.dtype)
            y_t = D_t.clone()

        if cum_D_rgb is not None:
            if cum_D_rgb.shape[2:] != (H, W):
                pooled_prev_r = F.adaptive_max_pool2d(cum_D_rgb.float(), (H, W))
                pooled_prev_t = F.adaptive_max_pool2d(cum_D_t.float(), (H, W))
            else:
                pooled_prev_r = cum_D_rgb.float()
                pooled_prev_t = cum_D_t.float()
            D_rgb = D_rgb * pooled_prev_r
            D_t = D_t * pooled_prev_t

        D_rgb, D_t = complementary_fix(D_rgb, D_t, y_rgb, y_t)

        cum_D_rgb = D_rgb.detach()
        cum_D_t = D_t.detach()

        all_D_rgb.append(D_rgb)
        all_D_t.append(D_t)
        all_y_rgb.append(y_rgb)
        all_y_t.append(y_t)
        ...
        for j, block in enumerate(blocks):
            ...
            quality_bias = _hard_mask_to_swin_bias(
                bias_combined, window_size, shift_size,
                neg_bias=attn_neg_bias)
    ...
    return outs, all_D_rgb, all_D_t, all_y_rgb, all_y_t


def _forward_swin_branch_hard(swin_branch, img, predictor_list,
                               training=True, gumbel_tau=1.0,
                               attn_neg_bias=-100.0):
    window_size = swin_branch.stages[0].blocks[0].attn.window_size
    outs, all_D, all_y = [], [], []
    ...
    for i, stage in enumerate(stages):
        ...
        if i < len(predictor_list) and predictor_list[i] is not None:
            x_2d = x.reshape(B_tok, H, W, -1).permute(0, 3, 1, 2)
            logits = predictor_list[i](x_2d)
            D, y = gumbel_softmax_hard(logits, tau=gumbel_tau, training=training)

        if D is None:
            D = torch.ones(B_tok, 1, H, W, device=x.device, dtype=x.dtype)
            y = D.clone()

        if cum_D is not None:
            if cum_D.shape[2:] != (H, W):
                pooled_prev = F.adaptive_max_pool2d(cum_D.float(), (H, W))
            else:
                pooled_prev = cum_D.float()
            D = D * pooled_prev
    ...
    return outs, all_D, all_y


@MODELS.register_module()
class SwinQualityHard(BaseSegmentor):
    def __init__(self,
                 backbone: ConfigType,
        ...
        self.attn_neg_bias = attn_neg_bias
        ...
        self._build_predictors(init_keep_bias)
    ...
    def _build_predictors(self, init_keep_bias):
        self.predictors_common_rgb = nn.ModuleList(
            [QualityPredictorGumbel(ch, init_keep_bias=init_keep_bias) for ch in self.embed_dims_list])
    ...
    def _extract_feat_single(self, rgb, t):
        ...
        for i in range(len(self.embed_dims_list)):
            ...
            if force:
                D_r_i = torch.ones(B, 1, zc_ri.shape[2], zc_ri.shape[3], device=zc_ri.device)
                D_t_i = torch.ones(B, 1, zc_ti.shape[2], zc_ti.shape[3], device=zc_ti.device)
                D_pr_i = torch.ones(B, 1, zp_ri.shape[2], zp_ri.shape[3], device=zp_ri.device)
                D_pt_i = torch.ones(B, 1, zp_ti.shape[2], zp_ti.shape[3], device=zp_ti.device)
            else:
                D_r_i = D_r[i]
                D_t_i = D_t[i]
                D_pr_i = D_pr[i]
        ...
        s_r = [y_r[i] for i in range(len(y_r))]
        s_t = [y_t[i] for i in range(len(y_t))]
        spr = [y_pr[i] for i in range(len(y_pr))]
        spt = [y_pt[i] for i in range(len(y_pt))]
        ...
        if self.loss_align_weight > 0:
            ...
            for i in range(len(zc_r)):
                if zc_r[i] is not None and zc_t[i] is not None:
                    D_r_i = s_r[i] if s_r[i] is not None else torch.ones(
                        B, 1, zc_r[i].shape[2], zc_r[i].shape[3], device=zc_r[i].device)
                    D_t_i = s_t[i] if s_t[i] is not None else torch.ones(
                        B, 1, zc_t[i].shape[2], zc_t[i].shape[3], device=zc_t[i].device)
                    both_keep = (D_r_i > 0.5).float().detach() * (D_t_i > 0.5).float().detach()
            ...
            if self.loss_align_weight > 0 and dzcr is not None and dzct is not None:
                ...
                for i in range(len(dzcr)):
                    if dzcr[i] is not None and dzct[i] is not None:
                        dD_r_i = ds_r[i] if ds_r is not None and i < len(ds_r) and ds_r[i] is not None else torch.ones(
                            B, 1, dzcr[i].shape[2], dzcr[i].shape[3], device=dzcr[i].device)
                        dD_t_i = ds_t[i] if ds_t is not None and i < len(ds_t) and ds_t[i] is not None else torch.ones(
                            B, 1, dzct[i].shape[2], dzct[i].shape[3], device=dzct[i].device)
                        both_keep = (dD_r_i > 0.5).float().detach() * (dD_t_i > 0.5).float().detach()
                ...
                for i in range(len(zf)):
                    if zf[i] is not None and dzf is not None and i < len(dzf) and dzf[i] is not None:
                        if zf[i].shape == dzf[i].shape:
                            D_r_c = s_r[i] if s_r[i] is not None else torch.ones(
                                B, 1, zf[i].shape[2], zf[i].shape[3], device=zf[i].device)
                            D_t_c = s_t[i] if s_t[i] is not None else torch.ones(
                                B, 1, zf[i].shape[2], zf[i].shape[3], device=zf[i].device)
                            D_r_d = ds_r[i] if ds_r is not None and i < len(ds_r) and ds_r[i] is not None else torch.ones(
                                B, 1, zf[i].shape[2], zf[i].shape[3], device=zf[i].device)
                            D_t_d = ds_t[i] if ds_t is not None and i < len(ds_t) and ds_t[i] is not None else torch.ones(
                                B, 1, zf[i].shape[2], zf[i].shape[3], device=zf[i].device)
                            both_c = (D_r_c > 0.5).float().detach() * (D_t_c > 0.5).float().detach()
                            both_d = (D_r_d > 0.5).float().detach() * (D_t_d > 0.5).float().detach()
                            gate = both_c * both_d
