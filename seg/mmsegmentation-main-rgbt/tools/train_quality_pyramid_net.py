"""
QualityAwarePyramidNet 自监督预训练脚本

核心设计：
  - 纯卷积共享编码器 + 全局描述子 + 多尺度质量头
  - 输入单张RGB图像，输出4个尺度的质量分数图
  - 与分割ViT各阶段token网格严格对齐
  - 训练策略与v3/v4一致：合成失真 + 排序损失
  - 复用mmseg数据集和pipeline，与语义分割完全一致的数据加载

运行方式:
  cd /home/lh/code/QWSEG/seg/mmsegmentation-main-rgbt
  python tools/train_quality_pyramid_net.py \
      --dataset mfnet \
      --work-dir work_dirs/quality_pyramid_pretrain \
      --amp
...
from mmseg.datasets.transforms.rgbt_augmentation import RGBTModalDegradation
...
def train(args):
    ...
    with open(config_path, 'w') as f:
        f.write(f'Model: QualityAwarePyramidNet\n')
        ...
        for batch_idx, raw_batch in enumerate(pbar):
            ...
            with autocast(enabled=args.amp):
                ...
                if random.random() < 0.3:
                    rgb_const = torch.zeros_like(rgb_raw)
                    rgb_missing_norm = _renorm_from_01(rgb_const, rgb_mean, rgb_std)
                    q_rgb_missing = quality_net.forward_rgb(rgb_missing_norm)
                    loss_missing_rgb = sum(
                        (q ** 2).mean() for q in q_rgb_missing
                    ) / len(q_rgb_missing)
                    loss_rgb = loss_rgb + 2.0 * loss_missing_rgb / args.grad_accum_steps
                    del q_rgb_missing, rgb_missing_norm, rgb_const
                ...
                if random.random() < 0.3:
                    t_const = torch.zeros_like(t_raw)
                    t_missing_norm = _renorm_from_01(t_const, ir_mean, ir_std)
                    q_t_missing = quality_net.forward_thermal(t_missing_norm)
                    loss_missing_t = sum(
                        (q ** 2).mean() for q in q_t_missing
                    ) / len(q_t_missing)
                    loss_t = loss_t + 2.0 * loss_missing_t / args.grad_accum_steps
                    del q_t_missing, t_missing_norm, t_const
